...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-lg p-6">
        ...
        @if($availablePrograms->count() > 0)
            <form action="{{ route('coach.members.assignProgram', $member) }}" method="POST">
                @csrf
                
                <div class="space-y-4 mb-6">
                    <h3 class="text-lg font-medium text-gray-700">Mes programmes disponibles</h3>
                ...
                <div class="flex justify-end gap-4">
                    <a href="{{ route('coach.members.show', $member) }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                        Annuler
                    </a>
        ...
        @else
            <div class="text-center py-8">
                ...
                <p class="text-gray-600 mb-4">Tous vos programmes sont déjà assignés à ce membre ou vous n'avez pas de programmes actifs.</p>
                <a href="{{ route('coach.members.show', $member) }}" class="px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
                    Retour
                </a>
            </div>
        @endif
                    ...
                    @foreach($member->programs as $assignedProgram)
                        <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                            <div>
                                ...
                                <p class="text-sm text-gray-600">{{ $assignedProgram->description }}</p>
                            </div>
