    ...
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ route('admin.coaches.update', $coach->id) }}" method="POST" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <x-form-field label="Prénom" name="first_name" type="text" placeholder="Ex: Jean" value="{{ $coach->first_name }}" required />
                <x-form-field label="Nom" name="last_name" type="text" placeholder="Ex: Dupont" value="{{ $coach->last_name }}" required />
            </div>

            <x-form-field label="Email" name="email" type="email" placeholder="coach@gym.com" value="{{ $coach->email }}" required />
            <x-form-field label="Téléphone" name="phone" type="tel" placeholder="06 12 34 56 78" value="{{ $coach->phone }}" />
